...
export default function ModelB({ posicion, animacion, ...props }) {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF(
    "./assets/buster_drone/scene.gltf"
  );
  const { actions } = useAnimations(animations, group);
  const nome = "Start_Liftoff";
  let ts = animacion ? 0.3 : 0;
...
useGLTF.preload("./assets/buster_drone/scene.gltf");
